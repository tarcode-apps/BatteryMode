...
type
  ...
  private
    ...
    procedure RenameLinkClick(Sender: TObject);
  protected
    procedure MonitorChangeEnable2(Sender: IBrightnessMonitor; Enable: Boolean);
    procedure MonitorChangeName2(Sender: IBrightnessMonitor; EffectiveName: string);
  public
...
begin
  ...
  FMonitor.OnChangeName2 := MonitorChangeName2;
  ...
  FEnableCheckBox.Caption := Monitor.EffectiveName;
  FEnableCheckBox.Hint := Monitor.Description;
  FEnableCheckBox.ShowHint := not string.IsNullOrEmpty(Monitor.FriendlyName);
  ...
  FRenameLink := TStaticText.Create(FEnablePanel);
  FRenameLink.Align := alLeft;
  FRenameLink.AlignWithMargins := True;
  FRenameLink.Margins.SetBounds(16, 0, 0, 0);
  FRenameLink.Caption := TLang[2002]; // Rename
  FRenameLink.LinkMode := True;
  FRenameLink.TabStop := True;
  FRenameLink.OnClick := RenameLinkClick;
  FRenameLink.Parent := FEnablePanel;
  FRenameLink.Left := FEnableCheckBox.ExplicitWidth;
...
procedure TBrightnessMonitorSettingsPanel.RenameLinkClick(Sender: TObject);
var
...
procedure TBrightnessMonitorSettingsPanel.MonitorChangeName2(
  Sender: IBrightnessMonitor; EffectiveName: string);
begin
  FEnableCheckBox.Caption := EffectiveName;
  FEnableCheckBox.ShowHint := not string.IsNullOrEmpty(Sender.FriendlyName);
end;
